...
if 'announcements' not in st.session_state: st.session_state.announcements = safe_load_json(ANNOUNCEMENT_FILE)
if 'select_options' not in st.session_state: st.session_state.select_options = safe_load_json(SELECT_OPTIONS_FILE)
if 'login_list' not in st.session_state: st.session_state.login_list = list(USER_TO_SUPPLIER.keys()) + list(ADMIN_USERS)

# ===================== 左侧登录 =====================
with st.sidebar:
    st.header("🔐 系统登录")
    login_options = st.session_state.login_list.copy()
    if "" not in login_options: login_options.append("")
    default_index = 0
    username = st.selectbox("选择用户名", options=login_options, index=default_index)
