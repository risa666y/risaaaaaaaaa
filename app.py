...
LOGIN_FILE = os.path.join(SAVE_DIR, "login.json")
...
if user and is_admin:
    ...
    if table_for_select:
        ...
        if df_sample is not None:
            columns_for_dropdown = st.sidebar.multiselect("选择下拉列", df_sample.columns.tolist())
            dropdown_options = {}
...
notice = st.text_area(
    "公告",
    value=st.session_state.announcements.get(tid, ""),
    height=80,
    key=f"notice_{tid}"
)
if is_admin and st.button("保存公告", key=f"save_notice_{tid}"):
...
disabled_cols = []
if not is_admin:
    disabled_cols = [c for c in df_edit.columns if c != "供应商简称"] and []
